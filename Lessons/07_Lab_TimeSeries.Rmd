...
Rainfall_BR_processed <-
  ...
  mutate( Date = my(paste0(Month,"-",Year))) %>% # paste0 = there is no seperator between the columns being joined 
...
# not good to use column bind in time series - use join function instead
# inner-join - R will assign NA to spots with no data 
...
BRA_tempTS <- ts(BRA_complete$Temperature_C, start = c(fyear,fmonth), frequency = 12)
BRA_rainTS <- ts(BRA_complete$Rainfall, start = c(fyear,fmonth), frequency = 12)
...
BRA_temmp_decom <- stl(BRA_tempTS, s.window = "periodic")
BRA_rain_decom <- stl(BRA_rainTS, s.window = "periodic")

plot(BRA_temmp_decom)
plot(BRA_rain_decom)
...
BRA_temp_Components <- as.data.frame(BRA_temmp_decom$time.series[,1:3])
BRA_rain_Components <- as.data.frame(BRA_rain_decom$time.series[,1:3])
...
CLASS NOTES:
3 components: trend, seasonality, and remainder (optional component: cyclical)
seasonal vs cyclical
  - seasonal: within 1 calendar year
  - cyclical: repeated pattern over a longer duration
stl function - receives a time series object and tries to identify the 3 components
if sum 3 components, get the original data 
cannot get rid of NAs - can drop that data and have a gap
trend detection methods 
  - scatter plot of time series does not show the movement of the series : use line plot 
cannot test for seasonal component with MK test 
  - need to use seasonal MK test
how to get rid of seasonal component
  - subtract seasonal comp from original data
  - can group seasonal data for yearly averages 
types of trends:
  1. deterministic
        - easy to observe, constant over time
        - can approx trend with a regression line
        - MK and Spearman
  2. stochastic
        - changes pace over time
