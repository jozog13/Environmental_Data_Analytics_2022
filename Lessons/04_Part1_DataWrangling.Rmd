...
colnames(NTL.phys.data) # gives all column names
head(NTL.phys.data) # gives first few observations(rows)
summary(NTL.phys.data) # summaries for each of the columns 
str(NTL.phys.data) # gives data type for each column
dim(NTL.phys.data) # gives data dimensions 
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] # look at surface information - only rows with a 0 in the depth column 
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") # choose all instances of Paul Lake or Peter Lake
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake") # get everything that is not the lakes listed
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) # depth in ascending order 
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) # depth in descending order
