...
wind_energy_huc8 <- 
  ...
  group_by(HUC_8) %>%
  summarize(WindPower = sum(kWhkm2day))         # these values should be smaller?? double check videos

#Join to HUC_8 features
huc8_sf_wind <- huc8_sf %>%
  left_join(wind_energy_huc8, by = 'HUC_8')
...
#View the outputs
mapview(huc8_sf_wind, zcol = "WindPower")
...
#Update the area the features, now that some are clipped        # ERROR HERE***
