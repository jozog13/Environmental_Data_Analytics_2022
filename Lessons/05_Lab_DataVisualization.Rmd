...
# paste - take multiple strings and put them together
# expression - allows us to put formatted characters into script
...
# x axis should be independent variable

plot1 <- 
  ggplot(filter(PeterPaul.chem.nutrients, depth==0)) +
  geom_point(aes(x=daynum,y=temperature_C,color=year4)) +
  facet_wrap("lakename") + #creates subplots with lake names
  ylab("Temperature (C)")
print(plot1)

# putting color inside the aes makes things different colors (if it was outside of aes, it would all be the same color)
